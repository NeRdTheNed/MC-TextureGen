...
public final class MC4k2Generator extends AbstractTextureGenerator {
     ...
     * Each block / texture ID is generated as texture group containing {@value #TEXTURES_PER_ID} textures,
    ...
    private static TextureGroup[] rawTextureDump() {
        ...
        for (int textureID = TEXTURE_OFFSET; textureID < MAX_TEXTURE_IDS; textureID++) {
            ...
            for (int subTexture = 0; subTexture < TEXTURES_PER_ID; subTexture++) {
                final BufferedImage tile = new BufferedImage(STANDARD_IMAGE_SIZE, STANDARD_IMAGE_SIZE, BufferedImage.TYPE_INT_ARGB);
                ...
                for (int yPixel = 0; yPixel < STANDARD_IMAGE_SIZE; yPixel++) {
                    for (int xPixel = 0; xPixel < STANDARD_IMAGE_SIZE; xPixel++) {
                        ...
                        case TEXTURE_LOG:
                            ...
                            // TODO refactor
                            if ((xPixel <= 0) || (xPixel >= (STANDARD_IMAGE_SIZE - 1)) || (yPixel <= 0) || (yPixel >= (STANDARD_IMAGE_SIZE - 1)) || (subTexture == 1)) {
                                ...
                                int tempOne = xPixel - ((STANDARD_IMAGE_SIZE / 2) - 1);
                                int tempTwo = (yPixel & STANDARD_IMAGE_SIZE_BITMASK) - ((STANDARD_IMAGE_SIZE / 2) - 1);
                        ...
                        case TEXTURE_GRASS:
                            ...
                            final int grassCheck = (((xPixel * xPixel * 3) + (xPixel * 81)) / 4) & ((STANDARD_IMAGE_SIZE / 4) - 1);

                            if ((yPixel + (subTexture * STANDARD_IMAGE_SIZE)) < (grassCheck + STANDARD_IMAGE_SIZE + (STANDARD_IMAGE_SIZE / 8))) {
                                color = COLOR_GREEN;
                                break;
                            }
                            ...
                            if ((yPixel + (subTexture * STANDARD_IMAGE_SIZE)) < (grassCheck + STANDARD_IMAGE_SIZE + (STANDARD_IMAGE_SIZE / 8) + 1)) {
                        ...
                        textureData[xPixel + (yPixel * STANDARD_IMAGE_SIZE)] =
                            (color            & 0xFF000000)                                               |
