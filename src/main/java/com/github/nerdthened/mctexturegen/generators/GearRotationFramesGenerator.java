...
public final class GearRotationFramesGenerator extends AbstractTextureGenerator {
    ...
    private static BufferedImage generateGearTextureForRotation(int rotationStep) {
        final BufferedImage rotatedImage = new BufferedImage(STANDARD_IMAGE_SIZE, STANDARD_IMAGE_SIZE, BufferedImage.TYPE_4BYTE_ABGR);
        ...
        for (int rotatedImageX = 0; rotatedImageX < STANDARD_IMAGE_SIZE; ++rotatedImageX) {
            for (int rotatedImageY = 0; rotatedImageY < STANDARD_IMAGE_SIZE; ++rotatedImageY) {
                ...
                final float gearImageX = ((rotatedImageX / (STANDARD_IMAGE_SIZE - 1.0F)) - 0.5F) * (ORIGINAL_GEAR_TEXTURE_SIZE - 1.0F);
                final float gearImageY = ((rotatedImageY / (STANDARD_IMAGE_SIZE - 1.0F)) - 0.5F) * (ORIGINAL_GEAR_TEXTURE_SIZE - 1.0F);
                ...
                if ((rotatedGearImageX >= 0) && (rotatedGearImageY >= 0) && (rotatedGearImageX < ORIGINAL_GEAR_TEXTURE_SIZE) && (rotatedGearImageY < ORIGINAL_GEAR_TEXTURE_SIZE)) {
                    final int gearDiv = STANDARD_IMAGE_SIZE / MIDDLE_GEAR_TEXTURE_SIZE;
                ...
                final int imageOffset = (rotatedImageX + (rotatedImageY * STANDARD_IMAGE_SIZE)) * 4;
                // Set ABGR values
